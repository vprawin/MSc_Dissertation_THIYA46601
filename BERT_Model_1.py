"""MSc Dissertation — BERT_Model_1.py

    BERT model variant #1: training script and helper functions.
...
for cls in rare_classes:
    df_cls = train_data[train_data['Precise Defect Description'] == cls]
    df_upsampled = resample(df_cls, replace=True, n_samples=min_samples, random_state=42)
    resampled_frames.append(df_upsampled)
...
# ==================== MODEL ====================
class BERT_Model_1(nn.Module):
    """BERT_Model_1 — class summary and usage notes."""
...
def collate_fn(batch):
    ...
    return {'input_ids': padded['input_ids'], 'attention_mask': padded['attention_mask'],
            'place': place, 'type': type_, 'labels': labels}
...
class CustomDataset(torch.utils.data.Dataset):
    ...
    """
        ...
        return {'input_ids': item['input_ids'], 'attention_mask': item['attention_mask'],
                'labels': item['labels'], 'place': torch.tensor(item['place']),
                'type': torch.tensor(item['type'])}
...
model = BERT_Model_1('bert-base-uncased', len(set(labels_encoded)), len(set(place_encoded)), len(set(type_encoded))).to(device)
...
def train_epoch(model, dataloader):
    ...
    for batch in tqdm(dataloader):
        ...
        outputs = model(inputs['input_ids'], inputs['attention_mask'], inputs['place'], inputs['type'])
