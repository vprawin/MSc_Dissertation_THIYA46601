...
BERT model variant #3 with categorical embeddings (place/type) and standardized
training/evaluation pipeline for dissertation:
...
def first_preprocessing(data: pd.DataFrame) -> pd.DataFrame:
    ...
    - Generate (a) processed text w/o NAME, and (b) BERT-ready lowercased text.
    """
    rows = data.copy()
...
resampled_frames = [
    resample(
        train_data[train_data['Precise Defect Description'] == cls],
...
def tokenize(batch_texts):
    return tokenizer(batch_texts, padding=True, truncation=True, max_length=128)
...
class BERT_Model_3(nn.Module):
    """
    BERT classifier with additional embeddings for categorical features (place/type).
    """
    def __init__(self, bert_model_name, num_labels, num_place, num_type, emb_dim=32, dropout=0.3):
        super().__init__()
        ...
        combined_dim = self.bert.config.hidden_size + 2 * emb_dim
    ...
    def forward(self, input_ids, attention_mask, place, type_):
        ...
        combined = torch.cat((bert_output, self.place_emb(place), self.type_emb(type_)), dim=1)
...
PARAM_GRID = [
    ...
    dict(emb_dim=32, dropout=0.1, lr=3e-5, epochs=8, patience=2),
]
...
def run_cv_once(cfg):
    ...
    for tr_idx, va_idx in skf.split(dev_texts, dev_labels):
        ...
        model = BERT_Model_3('bert-base-uncased', n_labels, n_place, n_type,
...
baseline_model = BERT_Model_3('bert-base-uncased', n_labels, n_place, n_type,
...
tuned_model = BERT_Model_3('bert-base-uncased', n_labels, n_place, n_type,
