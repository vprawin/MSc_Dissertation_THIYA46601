"""MSc Dissertation — BERT_Model_2.py

    BERT model variant #2: training script and helper functions.
...
# ==================== MODEL DEFINITION ====================
class BERT_Model_2(nn.Module):
    """BERT_Model_2 — class summary and usage notes."""
    ...
    """
        ...
        place_embedded = self.place_emb(place)
        type_embedded = self.type_emb(type_)
        combined = torch.cat((pooled_output, place_embedded, type_embedded), dim=1)
...
def collate_fn(batch):
    ...
    batch_encoding = tokenizer.pad({'input_ids': input_ids, 'attention_mask': attention_mask}, padding=True, return_tensors='pt')
    return {
